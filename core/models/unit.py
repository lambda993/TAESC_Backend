...
class Unit(CoreModel):
    ...
    max_slope = models.SmallIntegerField(verbose_name=_(
        'Max Slope'), help_text=_('Maximum slope this unit can be placed at.'))
    ...
    scale = models.DecimalField(verbose_name=_('Scale'), help_text=_(
        'How much a unit is scaled compared to its real dimensions.'), max_digits=20, decimal_places=10, null=True, blank=True)
...
class UnitBuilding(CoreModel):
    ...
    build_angle = models.IntegerField(verbose_name=_(
        'Build Angle'), help_text=_('Angle at which this building is placed'))
...
class UnitKamikaze(CoreModel):
    ...
    class Meta:
        verbose_name = _('Kamikaze Unit')
        verbose_name_plural = _('Kamikaze Units')
        ordering = ('-game_version', 'unit')
        unique_together = ('game_version', 'unit')
...
class UnitWater(CoreModel):
    ...
    max_water_depth = models.SmallIntegerField(default=0, verbose_name=_(
        'Max Water Depth'), help_text=_('Maximum depth this unit can go underwater.'))
    min_water_depth = models.SmallIntegerField(default=0, verbose_name=_(
        'Min Water Depth'), help_text=_('Minimum water depth this unit must be.'))
...
class UnitEconomySpecial(CoreModel):
    ...
    wind_generator = models.SmallIntegerField(verbose_name=_('Wind Generator'), help_text=_(
        'Rate of energy generated based on wind speed.'), default=0)
    tidal_generator = models.SmallIntegerField(verbose_name=_('Tidal Generator'), help_text=_(
        'Rate of energy generated based on water tide speed.'), default=0)

    def __str__(self):
        return self.unit.unit_name

    class Meta:
        verbose_name = _('Unit Special Economy')
        verbose_name_plural = _('Unit Special Economies')
        ordering = ('-game_version', 'unit')
        unique_together = ('game_version', 'unit')
...
class UnitAircraft(CoreModel):
    ...
    can_land = models.BooleanField(verbose_name=_('Can Land'), default=True)
    hover_attack = models.BooleanField(verbose_name=_('Hover Attack'), help_text=_(
        'This unit will hover around its target while attacking.'), default=False)
    cruise_altitude = models.SmallIntegerField(verbose_name=_(
        'Cruise Altitude'), help_text=_('Altitude at which this unit will be flying.'))
    attack_run_length = models.SmallIntegerField(verbose_name=_(
        'Attack Run Length'), help_text=_('Bombing run length.'), default=0)
    bank_scale = models.DecimalField(verbose_name=_('Bank Scale'), help_text=_(
        'Unit scale while rotating horizontally.'), max_digits=20, decimal_places=10, default=0)
    pitch_scale = models.DecimalField(verbose_name=_('Brake rate'), help_text=_(
        'Unit scale while rotating vertically.'), max_digits=20, decimal_places=10, default=0)

    def __str__(self):
        return self.unit.unit_name

    class Meta:
        verbose_name = _('Aircraft Unit')
        verbose_name_plural = _('Aircraft Units')
        ordering = ('-game_version', 'unit')
        unique_together = ('game_version', 'unit')
